...
public sealed class ReflectionGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<CommandHandlerInfo> commandHandlers)
    {
        ...
        builder.Append($$"""
            ...
            /// <summary>
            /// Source generated reflection data for request DTOs in the [{{assemblyName}}] assembly.
            /// </summary>
    ...
    private static void GenerateFileHeader(StringBuilder builder, TypeCollector collector, bool hasCommandHandlers)
    {
        ...
        var typeAliases = string.Join("\n", collector.Types.Select(t => $"using {t.TypeAlias} = {t.UnderlyingTypeName};"));
        var diagnosticsUsing = hasCommandHandlers ? "\nusing System.Diagnostics.CodeAnalysis;" : "";
        ...
        builder.Append($$"""
            ...
            using System.Runtime.CompilerServices;{{diagnosticsUsing}}
    ...
    private static void GenerateRegisterCommandExecutorsMethod(StringBuilder builder, ImmutableArray<CommandHandlerInfo> commandHandlers)
    {
        ...
        // Dynamic section - command handler registrations
        foreach (var handler in commandHandlers)
        {
            builder.Append($$"""
                        // {{handler.CommandType}} -> {{handler.HandlerType}}
                        registry[typeof({{handler.CommandType}})] = new(typeof({{handler.HandlerType}}))
                        {
                            HandlerExecutor = new global::FastEndpoints.CommandHandlerExecutor<{{handler.CommandType}}, {{handler.ResultType}}>(
                                sp.GetService<global::System.Collections.Generic.IEnumerable<global::FastEndpoints.ICommandMiddleware<{{handler.CommandType}}, {{handler.ResultType}}>>>()
                                ?? global::System.Array.Empty<global::FastEndpoints.ICommandMiddleware<{{handler.CommandType}}, {{handler.ResultType}}>>())
                        };
