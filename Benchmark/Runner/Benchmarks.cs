...
public class Benchmarks
{
    ...
    static HttpClient FastEndpointSourceGenClient { get; } = new WebApplicationFactory<FEBenchSourceGen.Program>().CreateClient();
    ...
    [Benchmark]
    public Task FastEndpointsSourceGen()
    {
        var msg = new HttpRequestMessage
        {
            Method = HttpMethod.Post,
            RequestUri = new($"{FastEndpointSourceGenClient.BaseAddress}benchmark/sourcegen/123"),
            Content = _payload
        };

        return FastEndpointSourceGenClient.SendAsync(msg);
    }
